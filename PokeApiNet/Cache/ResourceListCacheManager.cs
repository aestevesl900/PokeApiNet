...
namespace PokeApiNet.Cache
{
    ...
    internal sealed class ResourceListCacheManager : BaseCacheManager
    {
        ...
        /// <summary>
        /// Gets the <see cref="List{T}"/> stored in cache associated to a URL.
        /// </summary>
        ...
        /// <returns>
        /// A <see cref="List{T}"/> in case of a cache hit; null otherwise.
        /// </returns>
        public List<T> GetApiResourceList<T>(string url) where T : ApiResource
        {
            System.Type resourceType = typeof(T);
            return listCaches[resourceType].Get<T>(url) as List<T>;
        }
