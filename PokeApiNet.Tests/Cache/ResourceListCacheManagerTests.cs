...
namespace PokeApiNet.Tests.Cache
{
    public class ResourceListCacheManagerTests
    {
        ...
        public void GetApiResourceList_WithStoredUri_ReturnsResource()
        {
            ...
            (string url, List<Machine> list) = CreateFakeApiResourceList<Machine>();
            sut.Store(url, list);

            // act
            List<Machine> cached = sut.GetApiResourceList<Machine>(url);
        ...
        public void GetApiResourceList_WithNonStoredUri_ReturnsNull()
        {
            ...
            // act
            List<Machine> cached = sut.GetApiResourceList<Machine>(testUrl);
        ...
        public void AllCacheIsCleared()
        {
            ...
            (string machineUri, List<Machine> machineList) = CreateFakeApiResourceList<Machine>();
            ...
            List<Machine> cachedMachineList = sut.GetApiResourceList<Machine>(machineUri);
        ...
        private (string, List<T>) CreateFakeApiResourceList<T>(string url = null)
            where T : ApiResource
        {
            return (url ?? typeof(T).Name, new List<T>());
        }
